...
export class WrapperUserDelegatedService {
  ...
  getDelegatedEventLogs(  pageSize: number,currentPage: number, id:number): Observable<any> {
    pageSize = pageSize <= 0 ? 10 : pageSize;
      const url = `${this.Usersurl}/v1/delegate-user-auditevents?pageSize=${pageSize}&currentPage=${currentPage}&id=${id}`;
